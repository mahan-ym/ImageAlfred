...
change_color_objects_hsv_tool = gr.Interface(
    ...
    inputs=[hsv_df_input,gr.File(label="Input Image", file_types=["image"], type="binary")],
    outputs=gr.Image(),
    title="Image Recolor tool (HSV)",
    description="This tool allows you to recolor objects in an image using the HSV color space. You can specify the hue and saturation scale for each object.",
)

change_color_objects_lab_tool = gr.Interface(
    fn=change_color_objects_lab,
    inputs=[lab_df_input,gr.File(label="Input Image", file_types=["image"], type="binary")],
    outputs=gr.Image(),
    title="Image Recolor tool (LAB)",
    description="Recolor an image based on user input using the LAB color space. You can specify the new_a and new_b values for each object.",
)

privacy_preserve_tool = gr.Interface(
    fn=privacy_preserve_image,
    inputs=[gr.Textbox("user_input"),gr.File(label="Input Image", file_types=["image"], type="binary")],
    outputs=gr.Image(),
...
demo = gr.TabbedInterface(
    [change_color_objects_hsv_tool, change_color_objects_lab_tool, privacy_preserve_tool],
